...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://job-portal-server-u5ru.onrender.com/api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`https://job-portal-server-u5ru.onrender.com/api/v1/job/${params.id}`);
        if (res.data.success) {
  ...
  return (
    <div className='max-w-7xl mx-auto my-10'>
      ...
      <div>
        ...
        <h1 className='font-bold my-1'>Requirements: <span className='pl-4 font-normal text-gray-800'>{singleJobById?.requirements}</span></h1>
        <h1 className='font-bold my-1'>Experience: <span className='pl-4 font-normal text-gray-800'>{singleJobById?.experienceLevel} years</span></h1>
